...
// The save itself (insert + reminder + embed, in one transaction) lives in services/notes-write
...
export async function storeNote(req, res) {
  ...
  const note = await createNote({ userId: req.user.id, content: noteText, title, remindAt });
