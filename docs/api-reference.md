...
| POST   | `/api/delete-user`         | `deleteUser`        | `{ userId }` (**admin**). Purges the user's notes/profile (tx), deletes the Clerk identity, then best-effort removes their Qdrant note vectors. |
