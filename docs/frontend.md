...
- Ambient skeuomorphic touches: the page is **line-less paper** now — `.nb-paper` /
  ...
  background (`--nb-grain`, blended via the per-mode **`--nb-grain-blend`** token). `index.css`
  holds a **lit-relief paper-texture library** (`--tex-*`) generated by
  `frontend/scripts/gen-paper-textures.ts` — each is a `feTurbulence` height field lit by
  `feDiffuseLighting` (real directional tooth, not flat speckle) then desaturated so the palette
  hue comes only from `--background`. Each palette picks a **page grain + a finer sidebar "cover
  stock" from one family**: `classic` warm fractal `grain`, `paper` clean cold-press `tooth`,
  `copper` coarse hand-made `rough`, `sage` vertical `laid` lines, `warm` woven `linen` crosshatch
  (`--nb-grain` / `--nb-grain-sidebar`). The library is defined **twice — once per mode** (in the
  light `:root` scope and the `.dark` scope): **light** papers are bright and blended **`multiply`**
  (soft-light collapses on near-white, which is why light mode used to read flat), **dark** papers
  centre on mid-grey and blend **`soft-light`**. A palette just maps the family and inherits the
  right variant + blend by mode; retune amplitude/scale in the generator, not the data-URIs. The
  sidebar carries the family's `-fine` cut (`--nb-grain-sidebar`) — a quieter cover stock. Two
  ...
  (loops along a top edge — crowns the note-editor dialog as a top-bound steno pad). Type is
  trimmed to **2 loaded faces + 1 logo face** (Google Fonts `<link>` in `index.html`, Greek
  coverage): **Inter** (sans/UI/body, `--font-sans`) and **Source Sans 3** — ONE light humanist
  sans now doing **both** section/note titles (`--font-serif`) and Lexi's answers (`--font-reading`;
  `.chat-md` + the reasoning disclosure). The `--font-serif`/`-reading` token names are legacy —
  they render a **sans**, not a serif. **Archivo Black** is the **Mneme** logo wordmark ONLY
  (`--font-display`, Latin-only). Mono labels/code (`--font-mono`, charts, `❯`) ride the **system
  monospace stack** — no webfont. (Alegreya, Literata + JetBrains Mono retired to keep the count down.)
